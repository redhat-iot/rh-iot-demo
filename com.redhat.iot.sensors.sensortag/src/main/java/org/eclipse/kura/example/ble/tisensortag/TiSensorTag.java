...
public class TiSensorTag implements BluetoothLeNotificationListener {
        ...
        try {
            if (CC2650)
                firmwareVersion = hexAsciiToString(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_FIRMWARE_REVISION_2650));
            else {
                String firmware = m_bluetoothGatt
        ...
        try {
            if (CC2650)
                temperatures = calculateTemperature(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_TEMP_SENSOR_VALUE_2650));
            else
                temperatures = calculateTemperature(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_TEMP_SENSOR_VALUE_2541));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return temperatures;
    ...
    public double[] readTemperatureByUuid() {
        ...
        try {
            temperatures = calculateTemperature(
                    m_bluetoothGatt.readCharacteristicValueByUuid(TiSensorTagGatt.UUID_TEMP_SENSOR_VALUE));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return temperatures;
        ...
        try {
            if (CC2650)
                acceleration = calculateAcceleration(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_MOV_SENSOR_VALUE_2650));
            else
                acceleration = calculateAcceleration(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_ACC_SENSOR_VALUE_2541));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return acceleration;
        ...
        try {
            if (CC2650)
                return calculateAcceleration(
                        m_bluetoothGatt.readCharacteristicValueByUuid(TiSensorTagGatt.UUID_MOV_SENSOR_VALUE));
            else
                return calculateAcceleration(
                        m_bluetoothGatt.readCharacteristicValueByUuid(TiSensorTagGatt.UUID_ACC_SENSOR_VALUE));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return acceleration;
        ...
        try {
            if (CC2650)
                humidity = calculateHumidity(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_HUM_SENSOR_VALUE_2650));
            else
                humidity = calculateHumidity(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_HUM_SENSOR_VALUE_2541));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return humidity;
    ...
    public float readHumidityByUuid() {
        ...
        try {
            humidity = calculateHumidity(
                    m_bluetoothGatt.readCharacteristicValueByUuid(TiSensorTagGatt.UUID_HUM_SENSOR_VALUE));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return humidity;
    ...
    public float[] readMagneticField() {
        ...
        try {
            if (CC2650)
                ...
                magnetic = calculateMagneticField(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_MAG_SENSOR_VALUE_2541));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return magnetic;
    ...
    public float[] readMagneticFieldByUuid() {
        ...
        try {
            if (CC2650)
                ...
                magnetic = calculateMagneticField(
                        m_bluetoothGatt.readCharacteristicValueByUuid(TiSensorTagGatt.UUID_MAG_SENSOR_VALUE));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return magnetic;
    ...
    public String readCalibrationBarometer() {
        ...
        try {
            if (!CC2650) {
                if (firmwareRevision.contains("1.4"))
                    pressureCalibration = m_bluetoothGatt
        ...
        try {
            if (CC2650)
                pressure = calculatePressure(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_PRE_SENSOR_VALUE_2650));
            else if (firmwareRevision.contains("1.4"))
                pressure = calculatePressure(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_PRE_SENSOR_VALUE_2541_1_4));
            else
                pressure = calculatePressure(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_PRE_SENSOR_VALUE_2541_1_5));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return pressure;
    ...
    public double readPressureByUuid() {
        ...
        try {
            pressure = calculatePressure(
                    m_bluetoothGatt.readCharacteristicValueByUuid(TiSensorTagGatt.UUID_PRE_SENSOR_VALUE));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return pressure;
    ...
    public float[] readGyroscope() {
        ...
        try {
            if (CC2650)
                ...
                gyroscope = calculateGyroscope(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_GYR_SENSOR_VALUE_2541));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return gyroscope;
    ...
    public float[] readGyroscopeByUuid() {
        ...
        try {
            if (CC2650)
                ...
                gyroscope = calculateGyroscope(
                        m_bluetoothGatt.readCharacteristicValueByUuid(TiSensorTagGatt.UUID_GYR_SENSOR_VALUE));
        } catch (KuraException e) {
            s_logger.error(e.toString());
        }
        return gyroscope;
        ...
        try {
            if (CC2650)
                light = calculateLight(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_OPTO_SENSOR_VALUE_2650));
            else {
                s_logger.debug("Not optical sensor on CC2541.");
                light = 0.0;
            }
    ...
    public double readLightByUuid() {
        ...
        try {
            if (CC2650)
                light = calculateLight(
                        m_bluetoothGatt.readCharacteristicValueByUuid(TiSensorTagGatt.UUID_OPTO_SENSOR_VALUE));
            else {
                s_logger.debug("Not optical sensor on CC2541.");
                light = 0.0;
            }
        ...
        if (CC2650) {
            ...
            try {
                value = Integer.parseInt(
                        m_bluetoothGatt.readCharacteristicValue(TiSensorTagGatt.HANDLE_IO_SENSOR_VALUE_2650), 16)
                        | 0x01;
                hexValue = Integer.toHexString(value);
                m_bluetoothGatt.writeCharacteristicValue(TiSensorTagGatt.HANDLE_IO_SENSOR_VALUE_2650,
